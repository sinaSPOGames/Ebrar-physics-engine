...
func _process(delta):
	...
	translation = get_node(PlayerCharacterMesh).global_transform.origin
	var mesh_front = get_node(PlayerCharacterMesh).global_transform.basis.z
	var auto_rotate_speed =  (PI - mesh_front.angle_to($h.global_transform.basis.z)) * get_parent().get_parent().horizontal_velocity.length() * rot_speed_multiplier
